...
export default function GroupsPage() {
  ...
  const inputStyle: React.CSSProperties = {
    background: 'rgba(18,8,0,0.8)', border: '1px solid rgba(224,122,53,0.15)',
    borderRadius: 11, padding: '12px 14px',
    fontSize: 14, color: '#fdf0e8', outline: 'none', boxSizing: 'border-box', width: '100%',
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #0e0700 0%, #080300 60%, #0b0400 100%)' }}>
      <header style={{
        ...
        <span style={{ fontWeight: 800, fontSize: 16, color: '#fdf0e8', letterSpacing: '-0.02em' }}>내 크루</span>
        <div style={{ width: 48 }} />
      </header>
      ...
      <main style={{ maxWidth: 560, margin: '0 auto', padding: '28px 16px 100px' }}>
        <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
          ...
          <button onClick={joinGroup} style={{
            ...
            color: '#080400', fontSize: 13, fontWeight: 700, border: 'none', cursor: 'pointer', whiteSpace: 'nowrap',
        ...
        <button onClick={() => { setShowCreate(!showCreate); setError('') }} style={{
          ...
          background: 'transparent', border: '1px dashed rgba(224,122,53,0.2)',
        ...
        {showCreate && (
          <div style={{
            ...
            border: '1px solid rgba(224,122,53,0.18)', borderRadius: 18, padding: 18, marginBottom: 20, marginTop: 10,
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <button onClick={createGroup} style={{
                ...
                boxShadow: '0 4px 16px rgba(224,122,53,0.35)',
              }}>만들기</button>
            </div>
          </div>
        )}

        {error && <p style={{ color: '#e07a35', fontSize: 13, textAlign: 'center', marginBottom: 12 }}>{error}</p>}
        {msg && <p style={{ color: '#f09050', fontSize: 13, textAlign: 'center', marginBottom: 12, fontWeight: 700 }}>{msg}</p>}
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '52px 0' }}>
            <div style={{
              ...
              background: 'rgba(224,122,53,0.05)', border: '1px solid rgba(224,122,53,0.1)',
              display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 18px',
            }}>
              <svg width="26" height="22" viewBox="0 0 26 22" fill="none">
                <circle cx="9" cy="7" r="4" stroke="#4a2a10" strokeWidth="1.5"/>
                <circle cx="19" cy="8" r="3" stroke="#4a2a10" strokeWidth="1.5"/>
                <path d="M1 20c0-3.866 3.582-7 8-7s8 3.134 8 7" stroke="#4a2a10" strokeWidth="1.5" strokeLinecap="round"/>
                <path d="M18 16c2.761 0 5 1.567 5 3.5" stroke="#4a2a10" strokeWidth="1.5" strokeLinecap="round"/>
              </svg>
            </div>
            <p style={{ color: '#4a2a10', fontSize: 14, fontWeight: 700, marginBottom: 5 }}>참가한 크루가 없어요</p>
            <p style={{ color: '#2a1408', fontSize: 13 }}>크루를 만들거나 초대 코드로 참가해보세요</p>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {groups.map(g => (
              <Link key={g.id} href={`/groups/${g.id}`} style={{ textDecoration: 'none' }}>
                <div style={{
                  ...
                  <div>
                    <div style={{ fontSize: 16, fontWeight: 800, color: '#f0c4a0', marginBottom: 4 }}>{g.name}</div>
                    {g.description && <div style={{ fontSize: 13, color: '#4a2a10' }}>{g.description}</div>}
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6 }}>
                    {g.owner_id === userId && (
                      <span style={{ fontSize: 10, fontWeight: 800, color: '#e07a35', background: 'rgba(224,122,53,0.1)', border: '1px solid rgba(224,122,53,0.25)', padding: '2px 8px', borderRadius: 6 }}>
                        방장
                      </span>
                    )}
                    <span style={{ fontSize: 11, color: '#4a2a10', fontWeight: 700, letterSpacing: '0.1em' }}>{g.invite_code}</span>
                  </div>
                </div>
              </Link>
            ))}

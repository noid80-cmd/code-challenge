    ...
    <svg width={W} height={ROW_H} style={{ display: 'block', overflow: 'visible' }}>
      {[0,1,2,3,4].map(i => (
        <line key={i} x1={0} y1={PAD_T + i * LG} x2={W - 4} y2={PAD_T + i * LG}
          stroke="#2a1408" strokeWidth={1} />
      ))}
      {filled.map((chord, col) => {
        const x = PAD_L + col * MW
        return (
          <g key={col}>
            <line x1={x} y1={PAD_T} x2={x} y2={PAD_T + STAFF_H} stroke="#2a1408" strokeWidth={1} />
            {chord && (
              <text x={x + 6} y={PAD_T - 7} fontSize={12} fontWeight={700}
                fill="#c26020" fontFamily="-apple-system, BlinkMacSystemFont, 'Inter', 'Segoe UI', sans-serif">
                {chord}
              </text>
            )}
          </g>
      ...
      <line x1={PAD_L + 4 * MW} y1={PAD_T} x2={PAD_L + 4 * MW} y2={PAD_T + STAFF_H}
        stroke="#2a1408" strokeWidth={isLast ? 3 : 1} />
      {isLast && (
        ...
        <div style={{ position: 'relative', flex: 1 }}>
          <select
            ...
            style={{
              ...
              fontSize: 13, fontWeight: 600, color: '#c07840',
          ...
          <svg width="10" height="6" viewBox="0 0 10 6" style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }}>
            <path d="M1 1l4 4 4-4" stroke="#7a4820" strokeWidth="1.5" fill="none" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        </div>

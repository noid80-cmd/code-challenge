  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #060900 0%, #040600 60%, #070a00 100%)' }}>
      <header style={{
        ...
        <span style={{ fontWeight: 800, fontSize: 16, color: '#e8ffc0', letterSpacing: '-0.02em' }}>이번 주 랭킹</span>
        <div style={{ width: 48 }} />
      </header>
      ...
      <main style={{ maxWidth: 480, margin: '0 auto', padding: '32px 16px 100px' }}>
        <div style={{ textAlign: 'center', marginBottom: 36 }}>
          <span style={{
            ...
            color: '#7aaa18', padding: '5px 16px', borderRadius: 20,
          }}>
            {weekRange} · 좋아요 기준
          </span>
        </div>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '60px 0' }}>
            <p style={{ color: '#2e4008', fontSize: 14, fontWeight: 700 }}>이번 주 데이터가 없어요</p>
            <p style={{ color: '#182400', fontSize: 13, marginTop: 5 }}>챌린지에 참여하고 첫 번째가 되어보세요</p>
          </div>
        ) : (
          <>
            {/* Podium */}
                ...
                return (
                  <div key={idx} style={{ flex: 1, maxWidth: 120, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    <div style={{
                      ...
                      background: `linear-gradient(135deg, ${medalColors[idx]}, ${idx === 0 ? '#7acc00' : idx === 1 ? '#6b7280' : '#92400e'})`,
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      fontSize: isFirst ? 26 : 20, fontWeight: 900, color: '#050800',
                    ...
                    <div style={{ textAlign: 'center', marginBottom: 10 }}>
                      ...
                      <div style={{ fontSize: isFirst ? 13 : 12, fontWeight: 800, color: '#b8e860' }}>{u.name}</div>
                      <div style={{ fontSize: 11, color: '#506010', marginTop: 2 }}>♥ {u.total_likes}</div>
                    </div>
                    <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {weekly.slice(3).map((u, i) => (
                <div key={u.user_id} style={{
                  ...
                  border: '1px solid rgba(170,255,0,0.08)', borderRadius: 16,
                }}>
                  <span style={{ fontSize: 14, fontWeight: 900, color: '#182400', width: 22, textAlign: 'center' }}>
                    {i + 4}
                  </span>
                  <div style={{
                    ...
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontSize: 14, fontWeight: 800, color: '#050800', overflow: 'hidden', flexShrink: 0,
                  }}>
                    {u.avatar_url
                      ? <img src={u.avatar_url} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="" />
                      : u.name.slice(0, 1).toUpperCase()}
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 14, fontWeight: 800, color: '#b8e860' }}>{u.name}</div>
                    <div style={{ fontSize: 11, color: '#2e4008', marginTop: 2 }}>{u.submission_count}회 참여</div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: 16, fontWeight: 900, color: '#aaff00' }}>{u.total_likes}</div>
                    <div style={{ fontSize: 10, color: '#182400' }}>likes</div>
                  </div>
                </div>
              ))}
            </div>

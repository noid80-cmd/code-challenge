...
// AI가 고른 8개 마디를 그대로 쓰면 특정 위치에 같은 마디가 여러 생성에 걸쳐
// 반복되는 문제가 있었음. 각 마디를 박(2단위=1beat) 셀로 쪼갠 뒤 전체를
// 무작위로 섞어 재조립해서, 같은 8개 ID를 뽑아도 매번 다른 마디 구성이 나오게 한다.
...
- 같은 ID 최대 2번 반복 가능
- 모든 표기는 스트레이트(정박)로 연주됨. title·description 어디에도 스윙·셔플·그루브·펑크 등 특정 주법을 암시하는 장르/연주법 이름 사용 금지 — 실제 스윙 리듬으로 연주하라는 뜻으로 오해할 수 있음
...
JSON 객체로만 응답:
{
  ...
  const MELODY_BAR_PATTERNS: Record<string, string> = {
    ...
    '22': 'E2 c2 (3GFE D2',
    '23': 'CD/E/ F2 D2 C2', '24': 'C/DE/ G2 F2 D2', '25': 'C/D/E G2 E2 C2',
  ...
  type MelodyRecipeNeed = { leap: number; bigLeap: number; chromatic: number; rhythm: number; dotted: number; triplet: number; syncopation: number; rest: number }
  ...
  // (레시피별 반음 상한은 MELODY_RECIPES[].chromaticCap 참고)
  ...
  function validateMelodyCombined(allBars: string[], level: string, recipe: typeof MELODY_RECIPES[number]): string | null {
    ...
    if (countMelodyCategory(allBars, MELODY_CATEGORY.chromatic) > chromaticCap) return `chromatic count exceeds cap ${chromaticCap}`
  ...
  function addMelodyTiesAcrossBars(bars: string[]): string[] {
    const out = [...bars]
    const headNote = (bar: string) => bar.trim().split(/\s+/)[0]?.match(/\(?\d*([\^_=]?[A-Ga-g])/)?.[1]
    const tailNote = (bar: string) => {
      ...
      return n.startsWith('z') ? undefined : n.match(/([\^_=]?[A-Ga-g])/)?.[1]
    }
    for (let i = 0; i < out.length - 1; i++) {
      ...
      if (a.trim().split(/\s+/).pop()?.includes('(') || b.trim().split(/\s+/)[0]?.includes('(')) continue
...
[붓점 리듬 + 도약 38~39 — X는 순차진행만 있어서, 도약이 들어간 붓점을 추가.
정방향(">")과 역방향("<", 스코치 스냅) 둘 다 있음]

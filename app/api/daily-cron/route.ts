...
JSON 객체로만 응답:
{
  ...
  const MELODY_BAR_PATTERNS: Record<string, string> = {
    ...
    '8': 'C2 D E2 F D2', '9': 'C2 D E4 D', '11': 'z C D2 E2 D2', '14': 'C2 G2 E F2 D',
  ...
  const MELODY_CATEGORY = {
    neighbor: ['A', 'B', 'C', 'D'],
    leap: ['E', 'F', 'G', 'H', 'P', 'Q', 'R', 'S', 'T', '1', '2', '3', '4', '18', '20', '22'],
    bigLeap: ['1', '2', '3', '4', '20', '22'],
    chromatic: ['U', 'V', 'W'],
  ...
  function validateMelodyCombined(allBars: string[], level: string): string | null {
...
9: C2 D E4 D (긴 당김음: 붙임줄 없이 오프비트에서 바로 2분음표, 2박 지속)
...
도약: E,F,G,H,P,Q,R,S,T,1,2,3,4,18,20,22 (이 중 4도 이상 큰 도약: 1,2,3,4,20,22)
...
JSON 객체로만 응답:
{
  ...
  "patterns": [
    ...
    {"label": "당김음·도약", "bars": ["9", "17", "V", "1", "H", "P", "16", "6"]}
  ]
}`
    ...
    const MELODY_FALLBACK = {
      ...
      patterns: assembleMelodyABC([
        ...
        { label: '큰 도약·리듬 심화', bars: ['20', '20', 'U', '15', '2', '3', '8', '5'] },
      ])!,
    }
    ...
    for (let attempt = 1; attempt <= 10; attempt++) {
      ...
      if (!melodyRuleBroken) {
        ...
        melodyRuleBroken = validateMelodyCombined(combined, melodyLevel)
      }

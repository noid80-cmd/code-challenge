...
JSON 객체로만 응답:
{
  ...
  if (!existingMelody) {
    ...
    const melodyLevelRule = melodyLevel === 'advanced'
      ? '각 프레이즈에 도약 패턴(E,F,G,H,P,Q,R,S,T,1,2,3,4) 중 최소 5개(이 중 4도 이상 큰 도약 1~4 중 최소 2개 포함), 반음 패턴(U,V,W) 중 최소 1개, 리듬 심화 패턴(X,Y,Z) 중 최소 1개, 쉼표/붙임줄 패턴(5,6,7,8,9,10) 중 최소 1개 포함. 이웃음 진행 패턴(A,B,C,D)은 프레이즈당 최대 1개로 제한'
      : '각 프레이즈에 도약 패턴(E,F,G,H,P,Q,R,S,T,1,2,3,4) 중 최소 4개(이 중 4도 이상 큰 도약 1~4 중 최소 1개 포함), 반음 패턴(U,V,W) 중 최소 1개, 리듬 심화 패턴(X,Y,Z) 중 최소 1개, 쉼표/붙임줄 패턴(5,6,7,8,9,10) 중 최소 1개 포함. 이웃음 진행 패턴(A,B,C,D)은 프레이즈당 최대 2개로 제한'
...
JSON 객체로만 응답:
{
  ...
  "level": "${melodyLevel}",
  "patterns": [
    {"label": "큰 도약·반음 경과음", "bars": ["1", "E", "F", "U", "G", "5", "A", "X"]},
    {"label": "아르페지오·리듬 심화", "bars": ["2", "H", "P", "V", "Q", "Y", "8", "J"]}
  ]
}`

    const MELODY_FALLBACK = {
      title: '계이름 시창 챌린지',
      description: '큰 도약과 쉼표·붙임줄을 포함한 중급 챌린지입니다.',
      level: 'intermediate',

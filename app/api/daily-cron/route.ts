...
JSON 객체로만 응답:
{
  ...
  const MELODY_BAR_PATTERNS: Record<string, string> = {
    ...
    '5': 'C2 z2 E2 D2', '6': 'z2 G2 E2 C2', '7': 'G2 F2 z2 C2', '10': 'C2 G2 z2 E2',
  ...
  if (!existingMelody) {
    ...
    const melodyLevelRule = melodyLevel === 'advanced'
      ? '각 프레이즈에 도약 패턴(E,F,G,H,P,Q,R,S,T,1,2,3,4) 중 최소 4개(이 중 4도 이상 큰 도약 1~4 중 최소 2개 포함), 반음 패턴(U,V,W) 중 최소 1개, 리듬 심화 패턴(X,Y,Z,12,13) 중 최소 1개, 당김음 패턴(8,9,11) 중 최소 1개, 쉼표 패턴(5,6,7,10) 중 최소 1개 포함. 이웃음 진행 패턴(A,B,C,D)은 프레이즈당 최대 1개로 제한'
      : '각 프레이즈에 도약 패턴(E,F,G,H,P,Q,R,S,T,1,2,3,4) 중 최소 4개(이 중 4도 이상 큰 도약 1~4 중 최소 1개 포함), 반음 패턴(U,V,W) 중 최소 1개, 리듬 심화 패턴(X,Y,Z,12,13) 중 최소 1개, 당김음 패턴(8,9,11) 중 최소 1개, 쉼표 패턴(5,6,7,10) 중 최소 1개 포함. 이웃음 진행 패턴(A,B,C,D)은 프레이즈당 최대 2개로 제한'
...
JSON 객체로만 응답:
{
  ...
  "level": "${melodyLevel}",
  "patterns": [
    {"label": "큰 도약·리듬 심화", "bars": ["1", "E", "F", "G", "U", "X", "8", "5"]},
    {"label": "아르페지오·당김음", "bars": ["2", "H", "P", "Q", "V", "Y", "9", "6"]}
  ]
}`

    const MELODY_FALLBACK = {
      title: '계이름 시창 챌린지',
      description: '큰 도약과 쉼표, 당김음을 포함한 중급 챌린지입니다.',
      level: 'intermediate',
